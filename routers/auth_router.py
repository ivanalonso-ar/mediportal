    ...
    if tipo == "paciente":
        paciente = db.query(Paciente).filter(
            or_(
                ...
                Paciente.email == dni_email.strip().lower()
            ),
...
async def registro_post(
    ...
    if not RE_SOLO_LETRAS.match(nombre.strip()):
        return error("El nombre no puede contener números ni caracteres especiales.")
    if not RE_SOLO_LETRAS.match(apellido.strip()):
        return error("El apellido no puede contener números ni caracteres especiales.")
    if not RE_SOLO_NUMEROS.match(dni.strip()):
        return error("El DNI solo puede contener números, sin puntos ni espacios.")
    if telefono.strip() and not RE_SOLO_NUMEROS.match(telefono.strip()):
        return error("El teléfono solo puede contener números, sin guiones ni espacios.")
