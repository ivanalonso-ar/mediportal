...
create table if not exists grupos_familiares (
    id integer generated by default as identity primary key,
    titular_id integer not null references pacientes(id) on delete cascade,
    miembro_id integer not null references pacientes(id) on delete cascade,
    parentesco varchar(50),
    created_at timestamp with time zone not null default now(),
    constraint uq_grupo_familiar unique (titular_id, miembro_id)
);

create table if not exists solicitudes_grupo (
    id integer generated by default as identity primary key,
    solicitante_id integer not null references pacientes(id) on delete cascade,
    destinatario_id integer not null references pacientes(id) on delete cascade,
    parentesco varchar(50),
    estado varchar(20) not null default 'pendiente',
    created_at timestamp with time zone not null default now(),
    constraint solicitudes_grupo_estado_chk check (estado in ('pendiente', 'aceptada', 'rechazada'))
);
...
create table if not exists bonos (
    ...
    fecha varchar(20) not null,
    hora varchar(10) not null,
    emitido_por varchar(150) not null,
    observaciones text,
    estado varchar(20) not null default 'activo',
    created_at timestamp with time zone not null default now(),
    constraint bonos_estado_chk check (estado in ('activo', 'atendido', 'cancelado'))
);
