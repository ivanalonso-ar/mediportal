...
from horarios import PROFESIONALES

ESTADOS_ACTIVOS = ("pendiente", "confirmado")


def capacidad_especialidad(especialidad: str) -> int:
...
def hora_disponible(
    ...
    return len(mismos) < capacidad_especialidad(especialidad)
...
def horas_sin_disponibilidad(db: Session, fecha: str, especialidad: str) -> set[str]:
    ...
    cap = capacidad_especialidad(especialidad)
    conteo: dict[str, int] = {}
    for t in activos:
